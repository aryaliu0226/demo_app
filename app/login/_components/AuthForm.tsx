...
import { fetchLoginAction } from '@/app/_lib/actions/login'
...
export default function AuthForm() {
  ...
  const [closedGlobalErrorId, setClosedGlobalErrorId] = useState<number | null>(
    null,
  )
  const globalErrorId = state?.globalErrorId ?? null
  const globalErrorOpen =
    Boolean(state?.globalError) && globalErrorId !== closedGlobalErrorId

  return (
    <>
      <Modal
        title='操作失败'
        open={globalErrorOpen}
        okText='知道了'
        cancelButtonProps={{ style: { display: 'none' } }}
        onOk={() => setClosedGlobalErrorId(globalErrorId)}
        onCancel={() => setClosedGlobalErrorId(globalErrorId)}>
